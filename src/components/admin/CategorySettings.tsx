...
import { iconeDeCategorie } from "../../lib/categories";

// Gestion du référentiel des catégories, depuis l'onglet Configuration.
//
// Ajouter une catégorie demandait auparavant de modifier deux fichiers du code et de
// redéployer. C'est une décision éditoriale, pas un changement de logiciel.
//
// La CLÉ n'est jamais saisie : elle est dérivée du nom par le serveur. C'est ce qui garantit
// qu'elle reste normalisée — et donc qu'un filtre ne peut plus rater un événement à cause
// d'une majuscule ou d'un accent.
...
const ICONES_PROPOSEES = ["Tag", "Music", "PartyPopper", "Drama", "Trophy", "Briefcase", "LayoutGrid"];
...
export default function CategorySettings() {
  ...
  async function creer(e: React.FormEvent) {
    ...
    await appeler("/api/admin/categories", {
      method: "POST",
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="text-sm font-black text-gray-900">Catégories d'événements</h3>
        <p className="mt-1 text-[11px] leading-relaxed text-gray-500">
          Cette liste alimente à la fois les filtres de la page d'accueil et le choix proposé aux
          organisateurs : les deux ne peuvent plus diverger. Une catégorie masquée disparaît de ces
          listes sans toucher aux événements qui la portent.
        </p>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white">
        {chargement ? (
          ...
          <ul className="divide-y divide-gray-100">
            {categories.map((cat) => {
              const Icone = iconeDeCategorie(cat.icon);
                ...
                <li key={cat.slug} className={`flex flex-wrap items-center gap-3 px-4 py-3 ${cat.active ? "" : "bg-gray-50"}`}>
                  ...
                  {enRenommage ? (
                    ...
                    <div className="flex-1 min-w-[140px]">
                      ...
                      {/* La clé est montrée parce qu'elle apparaît dans les liens partagés
                          (?category=…) et ne change jamais, même après renommage. */}
                  ...
                  <div className="flex items-center gap-1">
                    {enRenommage ? (
                      <>
                        <button
                          onClick={() => {
                            appeler(`/api/admin/categories/${cat.slug}`, {
                              method: "PATCH",
                              headers: { "Content-Type": "application/json" },
                      ...
                      <>
                        ...
                        <button
                          onClick={() => appeler(`/api/admin/categories/${cat.slug}`, {
                            method: "PATCH",
                            headers: { "Content-Type": "application/json" },
                        ...
                        <button
                          onClick={() => {
                            if (!confirm(`Supprimer « ${cat.label} » ? Si des événements l'utilisent, elle sera simplement masquée.`)) return;
                            appeler(`/api/admin/categories/${cat.slug}`, { method: "DELETE" }, "Catégorie supprimée.");
                          }}
                          disabled={enCours}
